...
- GitHub API quirk observed in practice: `/repos/*` requests can 403 with "rate limit exceeded" for a few minutes even when `/rate_limit` shows quota remaining (separate anti-scraping bucket). Wait for the reset; don't chase phantom auth bugs.
